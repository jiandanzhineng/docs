...
用大模型把中文文档（`content/`）和 UI 文案（`i18n/`）翻译成 `en` / `ja` / `es` / `de`。
...
| `check-doc-translations.js` | **主力**：扫描 `content/`，翻译 `.md`/`.mdx` 与 `_category_.json`，含全套校验与图片同步 |
| `check-doc-links.js` | 检查 `content/` 与各 `i18n` locale 下所有 `.md`/`.mdx` 里指向**本地图片 / 资源 / 其它 md 的相对引用**是否存在（防「文件移目录后 `./img/xxx`、`../other/yyy.md` 忘了改」导致的线上 404 / 跳转失败） |
| `prune-orphan-translations.js` | 删除各 `i18n` locale 下**没有 `content/` 源对应物**的孤儿译文（源文档搬迁 / 删除后残留的旧翻译、旧图片），已接入 `npm run translate` 链自动执行 |
...
2. `check-doc-translations` — 翻译 `content/`
...
```bash
# 把 <file> 换成要抽查的文档（相对 content/ 的路径）
f="<file>"; s=$(grep -oE '\]\([^)]*\)' "content/$f" | wc -l); \
...
翻译完成后，工具会把 `content/` 中译文引用的本地图片自动复制到对应
`i18n/<locale>/.../` 目录。

- `[ASSET_COPIED]` 已复制
- `[ASSET_MISSING_SOURCE]` **源文档自己引用了不存在的图片**（源死链，与翻译无关，需在 `content/` 侧修复）

## 死链检查（`check-doc-links.js`）

扫描 `content/` + 全部 `i18n/<locale>/docusaurus-plugin-content-docs/current/`，提取每个
...
```bash
npm run check:doc-links                       # 扫 content/ + 全部 locale
...
`content/` 里已不存在的孤儿（旧 `.md`、旧图片、空目录），产生过时重复路由。本工具按
「`content/` 是唯一真相」清理：`i18n/<locale>/docusaurus-plugin-content-docs/current/` 下
每个文件，若 `content/` 里不存在同相对路径的源文件，即为孤儿，删除并清理空目录。
...
> 译文还引用着被删的孤儿（若有，说明该文件本该一起搬/改路径）。`zh-Hans` 用 `content/` 本身，
> 不参与清理。

## 已知问题

- `content/player/old/`、`content/player/client/` 等目录经历过搬迁，早期一批文档的 `./img/xxx`
