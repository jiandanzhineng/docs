...
本项目的网页文档内容主要存储在 `content/` 目录下。`docs/` 仅用于项目备忘（ADR、note）。

### 1.1 文件位置
- **网页文档源文件**：`content/` 目录。
- **项目备忘**：`docs/`（`docs/adr`、`docs/draft`、`docs/note.md`）。
- **博客文章**：`blog/` 目录。
- **页面**：`src/pages/` 目录。
...
1. **复制源文件**：
   将 `content/` 下的源文件复制到对应的语言目录中。
   例如，要翻译中文文档，需将文件复制到：
   `i18n/zh-Hans/docusaurus-plugin-content-docs/current/`

   ```bash
   # 示例：复制 content 目录到中文翻译目录
   cp -r content/* i18n/zh-Hans/docusaurus-plugin-content-docs/current/
   ```
